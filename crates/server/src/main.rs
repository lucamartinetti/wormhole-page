...
async fn security_headers(
    ...
    headers.insert(
        ...
        HeaderValue::from_static("default-src 'none'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; connect-src 'self' wss://relay.magic-wormhole.io ws: wss:; img-src 'self' data:; font-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"),
    );
    headers.insert(
